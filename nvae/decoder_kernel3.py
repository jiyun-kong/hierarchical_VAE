...
from nvae.common_kernel3 import Swish, DecoderResidualBlock, ResidualBlock
...
class Decoder(nn.Module):
    ...
    def forward(self, z, xs=None, mode="random", freeze_level=-1):
        ...
        for i in range(len(self.decoder_residual_blocks)):
            ...
            # 각 계층에서 이전 계층의 출력과 latent z를 합쳐서 다음 계층으로 전달
            ...
            # 특정 계층의 latent z만 고정하고 나머지 계층의 z는 샘플링해서 이미지 생성 가능
            # 예) 상위 계층 z 고정 : 전체적인 구조 (얼굴 형태 등)는 유지
            # 하위 계층 z 수정 : 세부적인 디테일 (머리카락, 표정 등)만 변함
